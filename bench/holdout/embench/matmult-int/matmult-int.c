...
/* HOLDOUT PORT: benchmark_body de-staticized (was `static int
   benchmark_body(...)`) so bench/holdout/embench/matmult-int/shim.c can
   call it directly with a fixed internal count of 1, bypassing the
   LOCAL_SCALE_FACTOR/GLOBAL_SCALE_FACTOR compounding that upstream's
   benchmark() uses for cross-benchmark runtime equalisation. The holdout
   harness controls repetition count itself via HOLDOUT_REPS in the
   wrapper loop instead. No other change to this function. */
int  benchmark_body(unsigned int lsf, unsigned int gsf);
